 ...
 */
static const char* stripwhitespace(char* buf) {
  char* pointer = buf;
  char* revpointer;
  ...
  while (revpointer > pointer && isspace(*revpointer)) *(revpointer--) = 0;
 ...
 */
static linkedlist* machinelist_lladd(linkedlist* machinelist,
 ...
 */
linkedlist* read_machinenetgroup(linkedlist* machinelist,
...
#define WORKAROUND_SETNETGRENT
  ...
  if (setnetgrent(groupname) WORKAROUND_SETNETGRENT) {
    ...
    while (getnetgrent(&hostp, &userp, &domainp)) {
      if (NULL != hostp) machinelist = machinelist_lladd(machinelist, hostp);
    }
    endnetgrent();
  } else {
    fprintf(stderr, _("%s: Unknown netgroup %s.\n"), PACKAGE, groupname);
 ...
 */
linkedlist* read_machinelist(linkedlist* machinelist, const char* listfile,
                             const char* alternatelistfile) {
  FILE* f;
  ...
  if ((f = fopen(listfile, "r")) ||
    ...
    if (alternatelistfile)
      fprintf(stderr, _("%s: File %s nor %s could not be opened for read\n"),
              PACKAGE, listfile, alternatelistfile);
    else
      fprintf(stderr, _("%s: File %s could not be opened for read\n"), PACKAGE,
              listfile);
    exit(1); /* It should fail on error */
  }
  free(buf);
...
int print_help(void) {
  ...
  printf(
      _("-v --verbose                   Verbose output\n"
        ...
        "-b --bufsize                   Change buffer size used in input "
        "duplication\n"
  ...
  if (f && (t = open_dshconfig(f, '='))) {
    for (line = t->config; line; line = line->next) {
      ...
      if (!strcmp(buf_a, "remoteshell")) {
        ...
        if (verbose_flag)
          printf(_("Setting wait-shell to  [%i]\n"), wait_shell);
      } else if (!strcmp(buf_a, "showmachinenames")) {
        pipe_option |= (atoi(buf_b) != 0);
        if (verbose_flag)
          printf(_("Setting pipe option to  [%i]\n"), pipe_option);
 ...
 */
static linkedlist* split_machines_list_and_add_machines(linkedlist* machinelist,
                                                        const char* optarg) {
 ...
 * @return 1 on failure
 */
static int sanity_checking(void) {
  if ((pipe_option & PIPE_OPTION_INPUT) && wait_shell) {
    fprintf(stderr,
            _("%s: Input duplication and concurrent shell need to be specified "
              "together\n"),
            PACKAGE);
    return 1;
  }
  ...
  if (forklimit < 0) {
    fprintf(stderr, "%s: %s", PACKAGE,
            _("Cannot specify fork limit of less than 0\n"));
    return 1;
  }

  if ((pipe_option & PIPE_OPTION_INPUT) && (forklimit > 0)) {
    fprintf(stderr,
            _("%s: Input duplication and concurrent shell without fork limit "
  ...
  if ((wait_shell == 1) && (forklimit > 0)) {
    fprintf(stderr,
            _("%s: fork limit and wait shell cannot be specified at the same "
              "time\n"),
            PACKAGE);
    return 1;
  }
 ...
 *
 * Remember to update other places, such as execute_rsh_multiple routine when
 * changing here.
 * @return 1 on failure.
 * @return 0 on success.
 */
int parse_options(int ac, char** av) {
  ...
  linkedlist* rshcommandline_r =
      NULL; /* command line for rsh in reverse order*/
...
#define EXTRAVALUE
  ...
  while ((c = getopt_long(ac, av, EXTRAVALUE "vqm:ar:f:g:hVcwo:MHn:ib:F:",
    ...
    switch (c) {
      case 'a':
        if (verbose_flag) printf(_("Adding all machines to the list\n"));
        {
          ...
          machinelist =
              read_machinelist(machinelist, buf, DSHCONFDIR "/machines.list");
          free(buf);
        }
        break;
      case 'g': {
        if ('@' == *optarg) { /* using libc call for using netgroup. */
                              /* +1 to skip @ */
          if (verbose_flag)
            printf(_("Adding netgroup %s to the list\n"), optarg + 1);
          machinelist = read_machinenetgroup(machinelist, optarg + 1);
        } else { /* using dsh's own method. */
          char *buf1, *buf2;
      ...
      case 'F':
        ...
        wait_shell = 0;
        if (verbose_flag)
          printf(_("Setting forklimit to %i and wait_shell to %i\n"), forklimit,
                 wait_shell);
        break;
      case 'v':
